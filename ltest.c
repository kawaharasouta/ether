...
int InitRawSocket(char *device, int promiscFlag, int ipOnly){
	...
	if(promiscFlag){
		...
		ifreq.ifr_flags = ifreq.ifr_falgs | IFF_PROMISC;
...
char *my_ether_ntoa_r(u_char *hwaddr, char *buf, socklen_t size){
	sprintf(buf, size, "%02x:%02x:%02x:%02x:%02x:%02x", hwaddr[0],hwaddr[1],hwaddr[2],hwaddr[3],hwaddr[4],hwaddr[5]);
	return buf;
}
...
<list>int PrintEtherHeader(struct ether_header *eh, FILE *fp){
	...
	fprintf(fp, "ether_shost = %s\n", myether_ntoa_r(eh->ether_shost, buf, sizeof(buf)));
